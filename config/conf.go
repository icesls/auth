...
type Config struct {
	...
	Traefik struct {
		...
		Enabled bool   `json:"enabled" yaml:"enabled" toml:"enabled"` // is or not resource traefik
		Domain  string `json:"domain" yaml:"domain" toml:"domain"`    // gateway domain
		Prefix  string `json:"prefix" yaml:"prefix" toml:"prefix"`    // if Prefix is not empty, it will resource router and middleware
